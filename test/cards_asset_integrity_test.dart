...
void main() {
  ...
  test('every card in assets/cards.json loads and is playable', () async {
    ...
    for (final card in cards) {
      expect(
        card.supportsLanguageMode(CardLanguageMode.bilingual),
        isTrue,
        reason: 'card ${card.id} does not have aligned bilingual content',
      );
      expect(
        card.isValid,
        isTrue,
        reason: 'card ${card.id} has ${card.answersEn.length} answers',
      );
      for (final locale in const [Locale('en'), Locale('es')]) {
        ...
        expect(
          ...
          reason: 'card ${card.id} ${locale.languageCode}',
        );
        expect(card.getPrompt(locale).isNotEmpty, isTrue);
        // points must come from the card, never the difficulty fallback
        for (final a in answers) {
          expect(
            card.answerPoints!.containsKey(a),
            isTrue,
            reason:
                'card ${card.id}: no explicit points for "$a" (${locale.languageCode})',
          );
        }
      }
      ...
      for (var index = 0; index < card.answersEn.length; index++) {
        expect(
          card.pointsForAnswer(card.answersEn[index]),
          card.pointsForAnswer(card.answersEs![index]),
          reason:
              'card ${card.id} answer $index scores differently by language',
        );
      }
    }
  });
